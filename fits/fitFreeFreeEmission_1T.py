...
def weights(T1):
    i = numpy.where(t == numpy.round(T1,0))
    #j = numpy.where(t == numpy.round(T2,0))
    #w = numpy.exp(-(t-T1)**2/w1) + r*numpy.exp(-(t-T2)**2/w2)
    w = numpy.zeros_like(t)
    w[i] = 1.0
    #w[j] = r
    return w/sum(w)
...
def penalty(parameters):
    ...
    return numpy.sum((weighted_spectra_sum(weights(T1))- normed_flux)**2)/n_spectra**2

def plot(parameters):
    fitted_spectrum = weighted_spectra_sum(weights(parameters))
    outtxt = zip(l_nm, fitted_spectrum)
    numpy.savetxt('fits/fit_%dpercent_T1.dat' % (power_percent), outtxt)
    outtxt = zip(l_nm, normed_flux)
    numpy.savetxt('fits/data_%dpercent_T1.dat' % (power_percent), outtxt)
...
#popt = optimize.minimize(penalty, x0=[10., 30., 1.], method='Nelder-Mead')
bounds=(10.0, 70.0)
popt = optimize.minimize_scalar(penalty, bounds=bounds)
